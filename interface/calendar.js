// JavaScript Document
		...
		for(var i = 0; i < Block.length; i++) {
			...
			if(Block[i].startTime > weekStart && Block[i].endTime < weekEnd) {
				...
				for (var m = 0; m <= 16; m++) {
					...
					if(newStartTime.getHours() == hour[m]) newTop = Math.round((0.01+(3*m))*1000)/1000;
				}
				top += Math.round((Block[i].startTime.getMinutes() * 0.04917)*1000)/1000;
				top += 'em';

				newTop += Math.round((newStartTime.getMinutes() * 0.04917)*1000)/1000;
				newTop += 'em';
				// height: 86px || 5.375em
				...
				newHeigth = (((newEndTime.getHours()	-	newStartTime.getHours()) 	* 32)+((newEndTime.getHours()	-	newStartTime.getHours())*1))-1;
				newHeigth 		+= (newEndTime.getMinutes()	-	newStartTime.getMinutes())	* 0.53;

				if(height <= 26) var info_text = "&nbsp;";
				//else if(height <= 32 && height > 22) 	 var info_text = "&nbsp;"; 
					else if(height <= 67 && height > 26) var info_text = "Von: "+formatCorrect(Block[i].startTime.getHours())+":"+formatCorrect(Block[i].startTime.getMinutes())+
																	" Uhr<br>Bis: "+formatCorrect(Block[i].endTime.getHours())+":"+formatCorrect(Block[i].endTime.getMinutes())+" Uhr";
						else var info_text = 	"Von: "+formatCorrect(Block[i].startTime.getHours())+":"+formatCorrect(Block[i].startTime.getMinutes())+
												" Uhr<br>Bis: "+formatCorrect(Block[i].endTime.getHours())+":"+formatCorrect(Block[i].endTime.getMinutes())+" Uhr";
	            var parentBlock = document.getElementById('datafields');
	            if(parentBlock != null) {
	            	var div = document.createElement("div");
	            	div.id = "TerminContainer"+i;
	            	div.className = "terminContainer";
	            	div.style.height = newHeigth+"px";
	            	div.style.left = left;
		            ...
		            div2.id = "Termin"+i;
		            div2.className = "termine";
		            div2.style.height = height+"px";
		            div2.style.left = left;
		            div2.style.top = top;
		            div2.style.cursor = "default";
		            div2.innerHTML = info_text;
		            parentBlock.appendChild(div2);
	...
	Date.prototype.createCalenedar = function(startDate, endDate, specialName, jsStart, jsEnd) {
		...
		if(document.getElementById("calendar") == null) {
			...
			writeDoc +=				Datafield;
			writeDoc +=			"</div>";
			writeDoc += 		"<div id='legende'><span id='legendIcon'>&nbsp;*&nbsp;</span><div style='max-width: 55em;'>Bitte beachten Sie, dass automatisch eine 45-Minütige Vor- und Nachbearbeitungszeit zu jedem Klausurblock hinzugefügt wird!</div>";
			writeDoc +=		"</div>";
